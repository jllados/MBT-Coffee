...
static int xref[32 + 1];      
char *aminoAcidCodes = "ABCDEFGHIKLMNOPQRSTUVWXYZ-";
char getAminoAcidCode(int i){return aminoAcidCodes[i];};

void setUpCrossReferences() {
	char c1, c2;
    	int i, j, maxRes, numRes = 32;  
	...
	for (i = 0; (c1 = amino_acid_order[i]); i++) {
		for (j = 0; (c2 = getAminoAcidCode(j)); j++) {
			if (c1 == c2) {
                		xref[i] = j;
				maxRes++;
               			break;
		...
		if ((xref[i] ==  - 1) && (amino_acid_order[i] != '*')) {
			fprintf ( stderr, "\nError:\nresidue %c in matrices.h is not recognised", amino_acid_order[i]);
        	}
	}
}
...
int * readUserMatrix(char *fileName)
{
	int numRes = 32;
    double f;
    FILE *fd;
	...
	userMat = (int *)malloc((1 + (numRes * numRes)) * sizeof(int));
	
	for (i = 0; i < 1 + (numRes * numRes); i++) {
        userMat[i] = 0;
    }
...
void getMatrix2(Constraint_list *CL, char *mat_name_in, int scale) {
	int numRes = 32, _maxAA = 24;
	int *matptr=NULL;
	int i, j, k, ix;
	int ti, tj;
	int maxRes;
	...
	matptr=readUserMatrix ( mat_name_in);

	if(CL->gmatrix) {
		//fprintf(stdout,"(INFO) Free CL->gmatrix\n");
		free(CL->gmatrix);
	}

    	CL->gmatrix= (int **)malloc(numRes * sizeof(int *));
    	for (i=0;i<numRes;i++)
            	CL->gmatrix[i] = (int *)malloc(numRes * sizeof(int));
...
ix = 0;
	...
	for (i = 0; i <= _maxAA; i++) {
        	...
        	for (j = 0; j <= i; j++) {
            		...
            		if ((ti !=  - 1) && (tj !=  - 1)) {
                		k = matptr[ix];
                		//k = matptr[ix] + (dsm[ix]/10);
                		if (ti == tj) {
                    			CL->gmatrix[ti][ti] = k * scale;
                    			maxRes++;
                		}else {
                    			CL->gmatrix[ti][tj] = k * scale;
                   			CL->gmatrix[tj][ti] = k * scale;
                		}ix++;
            		}
        	}
    	}
	...
	av1 /= (maxRes * maxRes) / 2;
    	av2 /= maxRes;
    	av3 = av3 / (((float)(maxRes * maxRes - maxRes)) / 2);
    	CL->matrixAvgScore =  - av3;

	min = max = CL->gmatrix[0][0];
    	for (i = 0; i <= _maxAA; i++)
    		for (j = 1; j <= i; j++) {
       			if (CL->gmatrix[i][j] < min) {
            			min = CL->gmatrix[i][j];
        		}
        		if (CL->gmatrix[i][j] > max) {
            			max = CL->gmatrix[i][j];
        		}
    		}
