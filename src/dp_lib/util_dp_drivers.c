...
int getRealLenght(Alignment*A, NT_node P) {
	int len = 0, seq, col, is;

	for (seq=0;seq<P->nseq; seq++) {
		is = 0;
    		for (col = 0; col < (int)strlen ( A->seq_al[P->lseq[seq]]); col++) {
				if (shift_aa(A->seq_al[P->lseq[seq]][col]-'A') != -20) {
					//fprintf(stdout,"%c ", A->seq_al[P->lseq[seq]][col]);
					is++;
				}
        	}
		//fprintf(stdout,"\n");
...
void setLogs (Constraint_list *CL, int len1, int len2) {

	if (len1 == 0 || len2 == 0) {
            CL->logmin = 1.0;
            CL->logdiff = 1.0;
        } else {
            CL->logmin = 1.0 / log10((double)MIN(len1, len2));
            if (len2 < len1) {
                CL->logdiff = 1.0 + 0.5 * log10((double)((float)len2 / (float)len1));
            } else if (len1 < len2) {
                CL->logdiff = 1.0 + 0.5 * log10((double)((float)len1 / (float)len2));
            } else {
                CL->logdiff = 1.0;
            }
            if (CL->logdiff < 0.9)
...
void readProfileAlignMatrix (Constraint_list *CL, NT_node ST, NT_node WS, int minLen) {
	int seq1, seq2;
	int  matAvgScore = 0, intScale = 100;
	float pcid = 0.0;
	char *mat_name_in;
	
	/*for (seq1=0;seq1<ST->nseq; seq1++) 
		for (seq2=0;seq2<WS->nseq; seq2++) {
			pcid += (float)(CL->DM)->similarity_matrix[ST->lseq[seq1]][WS->lseq[seq2]];
		}
	pcid = pcid / (float)WS->nseq;*/
	//fprintf (stdout, "(INFO) pcid: %f\n", pcid);
	
	CL->scale = 0.75;
	mat_name_in = "VTML200.mat";
	getMatrix2(CL, mat_name_in, intScale);	
}

int calcWeight(NT_node N) {
	return ((int)100);
	/*float weight = 0.0;
	
	weight += N->dist / N->leaf;
...
void ProfileStandard (Alignment*A, Constraint_list *CL, NT_node P, int prfLength, int** profile) {
	int aa, col, r, sum1, sum2, seq, i;
	...
	for (r = 0; r < prfLength; r++) {
		for (aa = 0; aa <= _maxAA; aa++) {
            ...
            for (seq=0;seq<P->nseq; seq++) {
    			if (aa == shift_aa(A->seq_al[P->lseq[seq]][r]-'A')) {
    				sum1+=calcWeight(P);
            	}
    		}
            profile[r + 1][aa] = (int)(10 *(float)sum1 / (float)sum2);
        }
    ...
    /*for (col = 0; col < prfLength; col++) {
    	for (aa = 0; aa <= _maxAA; aa++) {
     		fprintf(stdout,"%d\t", CL->prfstd[col + 1][aa]);
     	}
     	fprintf(stdout,"\n");	
	}*/
...
void ProfileWithSub (Alignment*A, Constraint_list *CL, NT_node P, int prfLength, int** profile) {
	int seq, sum2, col, aa, f, res, pos, i;
	float scale;
	
	int weighting[numRes + 2][prfLength + 2];
	memset(weighting, 0, sizeof(weighting[0][0]) * (numRes + 2) * (prfLength + 2));
	int gaps[prfLength + 2];
	memset(gaps, 0, sizeof(gaps));
	
	for (seq=0;seq<P->nseq; seq++) {
    	for (col = 0; col < (int)strlen ( A->seq_al[P->lseq[seq]]); col++) {
    		if (A->seq_al[P->lseq[seq]][col]-'A' == -20) {
            	gaps[col]++;
            }
        }
    }
	...
	int normWeight[P->nseq];
	for (seq=0;seq<P->nseq; seq++) {
    	normWeight[seq] = (calcWeight(P)/sum2) * 1000;
    }
	...
	for (col = 0; col < prfLength; col++) {
        for (aa = 0; aa <= _maxAA; aa++) {
            for (seq=0;seq<P->nseq; seq++) {
    			for (col = 0; col < (int)strlen ( A->seq_al[P->lseq[seq]]); col++) {
    				if (aa == shift_aa(A->seq_al[P->lseq[seq]][col]-'A')) {
						weighting[aa][col]+=calcWeight(P);
            		}
            	}
    		}
        }
		...
		for (pos = 0; pos < prfLength; pos++) {
			scale = (float)(P->nseq - gaps[pos]) / (float)P->nseq;
			for (res = 0; res <= _maxAA; res++) {
				f = 0;

				for (aa = 0; aa <= _maxAA; aa++) {
					f += (weighting[aa][pos] * CL->gmatrix[aa][res]);
				}
		...
		/*for (col = 0; col < prfLength; col++) {
			for (aa = 0; aa <= _maxAA; aa++) {
				fprintf(stdout,"%d\t", CL->prfsub[col + 1][aa]);
			}
     	fprintf(stdout,"\n");	
		}*/
		...
		/*for (pos = 0; pos < prfLength; pos++) {
			for (res = 0; res <= _maxAA; res++) {
				fprintf(stdout,"%d ", CL->prfsub[pos + 1][res]);
			}fprintf(stdout,"\n");
		}*/
	} 
...
void calcResidueSpecificPen(Alignment*A, NT_node P, int weight[], int prfLength) {
	int pascarellaProb[] = { 87, 87, 104, 69, 80, 139, 100, 104, 68, 79, 71, 137, 126, 93, 128, 124, 111, 75, 100, 77};
	int pascarellaRes[] = { 65, 67, 68, 69, 70, 71, 72, 75, 73, 76, 77, 78, 80, 81, 82, 83, 84, 86, 89, 87 };
	int _pascarellaNumRes = 20;
	char ix;
	int j, k;
	int i;
	
	//fprintf(stdout, "(INFO) calcResidueSpecificPen\n");

	for (i = 0; i < prfLength; i++) {
		weight[i] = 0;
		for (k=0;k<P->nseq; k++) {
			for (j = 0; j < _pascarellaNumRes; j++) {
				ix = shift_aa(A->seq_al[P->lseq[k]][i]-'A');
				if ((ix < 0) || (ix > _maxAA)) 
					continue;
				if (A->seq_al[P->lseq[k]][i] == pascarellaRes[j]) {
...
void calcHydrophilicPen(Alignment*A, NT_node P, int weight[], int prfLength) {
    int res, j, k, i, e, s;
    int hyd[prfLength + 2];
    
    char _hydResidues[] = {"GPSNDQEKR"};
	int numHydResidues = 9;
	
	//fprintf(stdout, "(INFO) calcHydrophilicPen\n");

	for (k=0;k<P->nseq; k++) {
        for (i = 0; i < prfLength; i++) {
            hyd[i] = 0;
            for (j = 0; j < numHydResidues; j++) {
            	res = shift_aa(A->seq_al[P->lseq[k]][i]-'A');
                if ((res < 0) || (res > _maxAA)) 
                    continue;
                if (A->seq_al[P->lseq[k]][i] == _hydResidues[j]) {
                    hyd[i] = 1;
                    break;
                }
            }
    ...
    for (i = 0; i < prfLength; i++) {
        weight[i] = (int)((weight[i] / P->nseq)); // Mark change 17-5-07
    }
}

int localPenalty(int penalty, int n, int resWeight[], int hydWeight[]) {
	int h = 0;
    float gw = 1.0;
...
void calcGapCoeff (Alignment*A, Constraint_list *CL, NT_node P, char* profile_name, int** profile) {
	int i, j, is, ie, prfLength;
	int gdist = 4;
	int gapCol = 32;
	int lenCol = 33;
	float reducedGap = 1.0;
	float scale;
	int gapCoef;
	int lenCoef;
	...
	if (CL->matrixAvgScore <= 0) {
			gapCoef = (int)(100.0 *(float)(gapOpen + CL->logmin));
	} else {
			gapCoef = (int)(CL->scale * CL->matrixAvgScore * (float)(gapOpen / (CL->logdiff * CL->logmin)));
	}
	lenCoef = (int)(100.0 * gapExtend);

	prfLength = ie = (int)strlen ( A->seq_al[P->lseq[0]]);
	int gaps[prfLength + 2];
	int gapPos[prfLength + 2];
	memset(gaps, 0, sizeof(gaps));
	memset(gapPos, 0, sizeof(gapPos));

	for (i=0;i<P->nseq; i++) {
		is = 0;

    	for (j = 0; j < prfLength; j++) {
    		if ( shift_aa(A->seq_al[P->lseq[i]][j]-'A') < 0 || shift_aa(A->seq_al[P->lseq[i]][j]-'A') > _maxAA)
    			is++;
    		else
		    	break;
    	}
	...
	while (i < prfLength) {
        if (gaps[i] <= 0) {
			...
			for (j = -gdist + 1; j < 0; j++) {
                if ((i + j >= 0) && (i + j < prfLength) && ((gaps[i + j] == 0) || (gaps[i + j] < j))) {     
                    gapPos[i + j] = j;
                }
            }
	...
	int resWeight[prfLength + 2];
	int hydWeight[prfLength + 2];
	memset(resWeight, 0, sizeof(resWeight));
	memset(hydWeight, 0, sizeof(hydWeight));
	calcResidueSpecificPen(A, P, resWeight, prfLength);
	
	/*for(i=0; i<prfLength + 2; i++)
    	fprintf(stdout,"%d ", resWeight[i]);
    fprintf(stdout,"\n");*/
	
	calcHydrophilicPen(A, P, hydWeight, prfLength);
	
	/*for(i=0; i<prfLength + 2; i++)
    	fprintf(stdout,"%d ", hydWeight[i]);
    fprintf(stdout,"\n");*/
	
	/*for(i=0; i<prfLength + 2; i++)
    	fprintf(stdout,"%d ", gapPos[i]);
    fprintf(stdout,"\n");*/

	if(strcmp(profile_name,"prfsub") == 0) {
		for (j = 0; j < prfLength; j++) {
			if (gapPos[j] <= 0) { // apply residue-specific and hydrophilic gap penalties.
				profile[j + 1][gapCol] = localPenalty(gapCoef, j, resWeight, hydWeight);
				profile[j+1][lenCol] = lenCoef;

				if (gapPos[j] < 0) { // increase gap penalty near to existing gaps.
					profile[j + 1][gapCol] = (int)((profile[j + 1][gapCol] * (2.0 + 2.0 * (gdist + gapPos[j]) / gdist)));
				}
			} else {
				scale = ((float)(P->nseq - gaps[j]) / (float)P->nseq) * reducedGap;
				profile[j + 1][gapCol] = (int)(scale * gapCoef);
				profile[j + 1][lenCol] = (int)(0.5 * lenCoef);
				}
		...
		/*for (int pos = 0; pos < prfLength; pos++) {
			for (int res = 0; res <= 33; res++) {
				fprintf(stdout,"%d ", CL->prfsub[pos + 1][res]);
			}fprintf(stdout,"\n");
		}*/
	
	} else if(strcmp(profile_name,"prfstd") == 0) {
		for (j = 0; j < prfLength; j++) {
			if (gapPos[j] <= 0) { // apply residue-specific and hydrophilic gap penalties.
				profile[j + 1][gapCol] = localPenalty(gapCoef, j, resWeight, hydWeight);
				profile[j+1][lenCol] = lenCoef;
		...
		/*for (int aa = 0; aa <= _maxAA; aa++) {
			for (int res = 0; res <= _maxAA; res++) {
				fprintf(stdout,"%d ", profile[aa][res]);
			}fprintf(stdout,"\n");
		}*/
	}
...
void generate_profiles (Alignment*A, Constraint_list *CL, NT_node L, NT_node R) {
	int len1;
	int len2;
	int lenprfsub=0, lenprfstd=0, i;

	len1 = getRealLenght(A, L);
	len2 = getRealLenght(A, R);
	
	readProfileAlignMatrix(CL, R, L, MIN(len1, len2));
	
	for (int seq=0;seq<L->nseq; seq++) {
		if(lenprfsub < (int)strlen ( A->seq_al[L->lseq[seq]]))
			lenprfsub = (int)strlen ( A->seq_al[L->lseq[seq]]);
	}
	CL->lenprfsub = lenprfsub;
	
	if(CL->prfsub) {
		free(CL->prfsub);
	}

    CL->prfsub = (int **)malloc((lenprfsub + 2) * sizeof(int *));
    for (i=0;i<(lenprfsub + 2);i++)
    	CL->prfsub[i] = (int *)malloc((numRes + 2) * sizeof(int));
	
	for (int seq=0;seq<R->nseq; seq++) {
		if(lenprfstd < (int)strlen ( A->seq_al[R->lseq[seq]]))
			lenprfstd = (int)strlen ( A->seq_al[R->lseq[seq]]);
	}
	CL->lenprfstd = lenprfstd;
	
	if(CL->prfstd) {
		free(CL->prfstd);
	}

    CL->prfstd = (int **)malloc((lenprfstd + 2) * sizeof(int *));
    for (i=0;i<(lenprfstd + 2);i++)
            CL->prfstd[i] = (int *)malloc((numRes + 2) * sizeof(int));
		
	if(L->nseq>=R->nseq) {
		setLogs(CL, len1, len2);
		ProfileStandard(A, CL, R, lenprfstd, CL->prfstd);
		ProfileWithSub(A, CL, L, lenprfsub, CL->prfsub);
		calcGapCoeff (A, CL, L, "prfsub", CL->prfsub);
		calcGapCoeff (A, CL, R, "prfstd", CL->prfstd);
	} else {
		setLogs(CL, len2, len1);
		ProfileStandard(A, CL, L, lenprfsub, CL->prfsub);
		ProfileWithSub(A, CL, R, lenprfstd, CL->prfstd);
		calcGapCoeff (A, CL, L, "prfstd", CL->prfsub);
		calcGapCoeff (A, CL, R, "prfsub", CL->prfstd);
	}
}
